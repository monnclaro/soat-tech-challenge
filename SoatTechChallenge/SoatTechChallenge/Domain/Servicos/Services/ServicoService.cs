...
public class ServicoService : IServicoService, ITransientService
{
    private readonly IRepository<Servico> _repository;

    private static ServicoResponse MapToResponse(Servico s) => new(s.Id, s.Nome, s.Descricao, s.Preco, s.TempoEstimadoMinutos);
    ...
    public async Task<ServicoResponse> Buscar(Guid id)
    {
        ...
        {
             throw new NotFoundException($"Servico de id: {id} não encontrado.");
        }
    ...
    public async Task<PagedResponse<ServicoResponse>> BuscarListaPaginada(PagedRequest request)
    {
        ...
        return new PagedResponse<ServicoResponse>(resultado);
    }
